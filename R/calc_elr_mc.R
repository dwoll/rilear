...
#####---------------------------------------------------------------------------
## helper function to pass one batch of parameter settings to get_elr()
#####---------------------------------------------------------------------------
...
get_elr_mc <- function(## parameters with uncertainty
    ...
    l_out <- if(multicore) {
        ...
        clusterEvalQ(cl, library(elrisk))
        # parallel::clusterExport(cl, c("f_latency", "get_elr"))
        ...
        lapply(l_param, get_elr, lat_method=lat_method, ...)
    }
    
    elr_mc <- unlist(l_out, recursive=TRUE)
    
    #####-----------------------------------------------------------------------
    ## summarize excess lifetime risk distribution?
    if(aggr_mc) {
        elr_ci <- quantile(elr_mc, probs=c(alpha/2, 1 - (alpha/2)), na.rm=TRUE)
        c(mean_rsk  =mean(elr_mc,   na.rm=TRUE),
          median_rsk=median(elr_mc, na.rm=TRUE),
          CIlo_rsk  =unname(elr_ci[1]),
          CIup_rsk  =unname(elr_ci[2]))
    } else {
        ## otherwise return MC results
        setNames(elr_mc, sprintf("MC%.6d", seq_len(n_sim)))
    }
}
