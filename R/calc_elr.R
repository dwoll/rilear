...
get_elr <- function(## parameters with uncertainty - in list l_param
    ...
    ddref          <- l_param[["ddref"]]
    lat_t0         <- l_param[["lat_t0"]]   # latency function: central value
    lat_eta        <- l_param[["lat_eta"]]  # latency function ProZES: width of transition period
    ...
    if(age_max <= max(agex)) {
        ...
        if(length(agex) == 0L) {
            return(setNames(NA_real_, metric))
        }
    }
    ...
    if(metric %in% c("REID", "REIC", "ELR", "RADS")) {
    ...
    f_elr_err <- if(metric %in% c("LAR", "LEAR", "CER")) {
        function(err) {
            (cancer_base*err*surv_base)[idx_keep]    / surv_base[idx_agex_1st]
        }
    } else if(metric %in% c("REID", "REIC")) {
        function(err) {
    ...
    f_elr_ear <- if(metric %in% c("LAR", "LEAR", "CER")) {
        function(ear) {
            (ear*surv_base)[idx_keep]    / surv_base[idx_agex_1st]
        }
    } else if(metric %in% c("REID", "REIC", "ELR")) {
        ## Note: ELR same EAR as REID / REIC
        function(ear) {
    ...
    if(metric == "RADS") {
        elr_err <- 1 - exp(-sum(f_elr_err(err_sum_a)))
        elr_ear <- 1 - exp(-sum(f_elr_ear(ear_sum_a)))
    } else {
        elr_err <- sum(f_elr_err(err_sum_a))
        elr_ear <- sum(f_elr_ear(ear_sum_a))
    }
    
    ## final excess risk based on selected ERR / EAR transfer
    elr_total <- weighted.mean(c(elr_err, elr_ear), w=wt_transfer)
    setNames(elr_total, metric)
}
